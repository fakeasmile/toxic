...
class CustomEmbeddings(BertEmbeddings):
    ...
    最终嵌入 = 原始嵌入 + gate_toxic(embeddings) * toxic_emb + gate_stance(embeddings) * stance_emb
    门控机制：以原始嵌入为输入，自适应学习每个位置/维度的融合权重（sigmoid约束到[0,1]）
    """
    def __init__(self, config, pretrained_embeddings, num_toxic_types=6):
        ...
        # 门控层：以原始嵌入为输入，输出每个位置/维度的融合权重
        self.gate_toxic = nn.Linear(config.hidden_size, config.hidden_size)
        self.gate_stance = nn.Linear(config.hidden_size, config.hidden_size)
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, **kwargs):
        ...
        # 门控融合：以原始嵌入为信号，自适应学习融合权重
        if toxic_ids is not None:
            toxic_emb = self.toxic_embeddings(toxic_ids)
            gate_toxic = torch.sigmoid(self.gate_toxic(embeddings))
            embeddings = embeddings + gate_toxic * toxic_emb

        if stance_ids is not None:
            stance_emb = self.stance_embeddings(stance_ids)
            gate_stance = torch.sigmoid(self.gate_stance(embeddings))
            embeddings = embeddings + gate_stance * stance_emb
