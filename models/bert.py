...
class BERTBaseline(nn.Module):
    ...
    def __init__(self, bert_path, num_classes=2, dropout_rate=0.1):
        ...
        """
        super().__init__()
        self.bert = BertModel.from_pretrained(bert_path)  # 加载预训练BERT
        self.dropout = nn.Dropout(dropout_rate)  # Dropout层
        self.classifier = nn.Linear(self.bert.config.hidden_size, num_classes)  # 分类头: 768 → num_classes
    ...
    def forward(self, input_ids, attention_mask, token_type_ids=None):
        ...
        return self.classifier(self.dropout(outputs.pooler_output))  # [CLS]池化输出 → Dropout → 分类
