...
### 实验5：embedding动态权重融合
* 变量：融合方式从 `embeddings += toxic_emb + stance_emb` 改为 `embeddings += sigmoid(a) * toxic_emb + sigmoid(b) * stance_emb`，a、b为可学习向量（768维，每维度独立），sigmoid约束到[0,1]
* 分支：experiment/emb-dynamic-weight
...
分析：
* 动态权重融合相比基线F1下降约1.57%，性能有所下降但远好于拼接+投影（下降6.6%）
* sigmoid(0)=0.5，初始化时权重约为0.5，相当于对额外嵌入做了一半的缩放，训练过程中模型学习调整比例

结论：
* 动态权重融合保留了残差连接结构（原始嵌入为主信号），因此性能下降远小于拼接+投影，验证了残差连接的重要性
* 但性能仍不如固定权重=1的基线，说明模型并不需要降低额外嵌入的权重——toxic_emb和stance_emb的信息在当前相加方式下已被充分利用，缩放反而削弱了信号
* 这进一步印证了实验1的结论：额外嵌入层必须可训练且信号不被压制，固定权重=1（即无缩放的直接相加）已是最优策略