...
2. 两阶段模式（--use_two_stage）：
   - Stage 1：让LLM通过假设检验分析文本的隐含意图，结果缓存
...
def parse_args():
    ...
    parser.add_argument('--use_two_stage', action='store_true',
                        help='启用两阶段推理：Stage1通过假设检验生成隐含意图分析，Stage2基于分析进行Likert评分')
...
# 两阶段模式Stage 1的提示词：假设检验+受约束推理+多视角验证
...
def generate_stage1_analysis(data_set, tokenizer, llm_model, is_qwen3=False, prompt_suffix="", cache_path=None):
    ...
    print(f"Stage 1：为 {len(data_set)} 条文本生成隐含意图分析...")
