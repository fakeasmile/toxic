...
基于语用推理量化作者以各形容词所描述方式表达态度的程度，构建可解释的概念向量。
方案B修正版（Pragmatic Intent Scoring）：用"表达"替代"实施"，
区分"讨论/引用/反对"与"表达态度"，解决长文本误检和伪装型毒性漏检问题。
...
# 提示词定义
# =============================================================================
# system instruction：引导LLM进行语用推理，评估"作者表达的态度"而非"文本相关程度"
# 方案B修正版（Pragmatic Intent Scoring）：基于PIC的Relevance Theory，
# 用"表达"替代"实施"，覆盖明示+暗示，区分"讨论/引用/反对"与"表达态度"
SYSTEM_INSTRUCTION = (
    "你是一位语用分析专家，擅长识别文本中作者的真实意图。\n"
    "在评估时，请进行语用推理：综合考虑文本的字面意思、比喻、谐音、反讽、文化暗示等隐含语义，\n"
    "判断作者在多大程度上以该形容词所描述的方式表达其态度或意图。\n"
    "请用1到5的数字评估，1表示作者完全没有该意图（仅客观陈述、引用或反对相关话题），"
    "5表示作者明确以该形容词所描述的方式表达其态度。只回答一个数字。"
)
