"""3级行为锚定Likert提示词模板调试工具（单样本切片分析，vLLM版本）

【定位】
本脚本是 generate_3level_concept_vllm.py 的"单样本切片"调试工具。
generate_3level_concept_vllm.py 负责为数据集中所有文本、所有形容词批量生成概念向量；
...
3. Verbalizer 概率分析（P(1)/P(2)/P(3)覆盖率）
4. P(3)归一化概念分数计算
5. 双维度分解：相关性 + 实施度

【与 generate_3level_concept_vllm.py 的关系】
- 本脚本的提示词构建逻辑、verbalizer 词表、分数计算逻辑与 generate_3level_concept_vllm.py 完全一致。

【使用方法】
直接修改下方 CONFIG 区域的变量，然后运行：
python scripts/inspect_3level_template_vllm.py
"""
import math
import os
import sys
...
def main():
    ...
    print("\n" + "=" * 60)
    print("3级行为锚定Likert模型推理调试（vLLM版本）")
    print("=" * 60)
    ...
    # 3级Likert Verbalizer分析
    level_ids = get_first_token_ids(verbalizer_words, tokenizer)

    print(f"\n3级Likert Verbalizer分析:")
    print(f"  1 (不相关) token ID: {level_ids[0]}")
    print(f"  2 (涉及)   token ID: {level_ids[1]}")
    print(f"  3 (实施)   token ID: {level_ids[2]}")

    level_probs = [probs_dict.get(tid, 0.0) for tid in level_ids]
    verbalizer_total = sum(level_probs)

    print(f"\n  P(1) 不相关 = {level_probs[0]:.6f}")
    print(f"  P(2) 涉及   = {level_probs[1]:.6f}")
    print(f"  P(3) 实施   = {level_probs[2]:.6f}")
    print(f"  Verbalizer概率总和 = {verbalizer_total:.6f}")
    print(f"  Verbalizer占总概率比例 = {verbalizer_total:.2%}")

    # 概念分数计算（与generate_3level_concept_vllm.py一致）
    print(f"\n{'=' * 60}")
    print("概念向量分数计算")
    print(f"{'=' * 60}")

    total = sum(level_probs) + 1e-8
    score = level_probs[2] / total
    print(f"P(3)归一化分数 = P(3) / (P(1)+P(2)+P(3)+ε) = {score:.6f}")

    # 双维度分解
    print(f"\n双维度分解:")
    relevance = (level_probs[1] + level_probs[2]) / total
    enactment = level_probs[2] / (level_probs[1] + level_probs[2] + 1e-8)
    print(f"  相关性 = (P(2)+P(3)) / 总概率 = {relevance:.6f}")
    print(f"  实施度 = P(3) / (P(2)+P(3)) = {enactment:.6f}")
    print(f"  解读: 概念{'相关' if relevance > 0.5 else '不相关'}，"
          f"说话者{'在实施' if enactment > 0.5 else '在涉及/提及'}该行为")
