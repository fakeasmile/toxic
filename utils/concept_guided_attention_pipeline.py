"""概念向量引导的注意力模型（ConceptGuidedBERT）训练与测试。
...
使用示例:
    # 1. 训练+测试
    python utils/concept_guided_attention_pipeline.py --mode all --dataset_name TOXICN --model_name Qwen2.5-3B-Instruct --template likert

    # 2. 仅训练模式
    python utils/concept_guided_attention_pipeline.py --mode train --dataset_name TOXICN --model_name Qwen2.5-3B-Instruct --template likert

    # 3. 仅测试模式 (必须指定实验时间戳)
    python utils/concept_guided_attention_pipeline.py --mode test --timestamp 20260415-085433 --dataset_name TOXICN --model_name Qwen2.5-3B-Instruct --template likert

    # 4. 自定义数据集和超参数（完整命令）
    python utils/concept_guided_attention_pipeline.py --mode all
        --dataset_name COLD
        ...
        --seed 42
    一般情况下：
    python utils/concept_guided_attention_pipeline.py --mode all --dataset_name TOXICN --model_name Qwen2.5-3B-Instruct --template likert --epochs 5 --use_deterministic

    # 5. 启用确定性模式 (确保实验可复现)
    python utils/concept_guided_attention_pipeline.py --mode all --dataset_name TOXICN --model_name Qwen2.5-3B-Instruct --template likert --use_deterministic --seed 42
...
输出文件:
    实验目录结构 (experiments_concept_guided_attention/<timestamp>/):
        ├── config.json              # 实验配置快照
...
from configs.concept_guided_attention_config import ConceptGuidedAttentionConfig
from models.bert import ConceptGuidedBERT

matplotlib.rcParams['font.sans-serif'] = ['SimHei', 'Microsoft YaHei', 'FangSong']


class ConceptGuidedDataset(Dataset):
    """概念引导模型数据集，封装tokenizer编码、概念向量和标签。"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="概念向量引导的注意力模型 训练与测试统一流水线",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  # 完整流水线 (训练+测试)
  python concept_guided_attention_pipeline.py --mode all --dataset_name TOXICN --model_name Qwen2.5-3B-Instruct --template likert

  # 仅训练
  python concept_guided_attention_pipeline.py --mode train --dataset_name TOXICN --model_name Qwen2.5-3B-Instruct --template likert

  # 仅测试
  python concept_guided_attention_pipeline.py --mode test --timestamp 20260415-085433 --dataset_name TOXICN --model_name Qwen2.5-3B-Instruct --template likert

  # 自定义超参数
  python concept_guided_attention_pipeline.py --mode all --dataset_name COLD --model_name Qwen2.5-3B-Instruct --template binary --epochs 10
        """
    )
...
def update_ConceptGuidedAttentionConfig(args):
    """基于ConceptGuidedAttentionConfig默认值，根据命令行参数更新配置对象。

    优先级: 命令行参数 > ConceptGuidedAttentionConfig默认值

    :param args: 命令行参数
    :return: 更新后的 ConceptGuidedAttentionConfig 对象
    """
    config = ConceptGuidedAttentionConfig()
...
def load_data(config, mode):
    ...
    :param config: ConceptGuidedAttentionConfig 配置对象
...
def tokenize_and_build_dataset(texts, labels, concepts, tokenizer, max_seq_length):
    """对文本进行tokenize编码，与概念向量一起构建ConceptGuidedDataset。
    ...
    :return: ConceptGuidedDataset 实例
    """
    encodings = tokenizer(
    ...
    return ConceptGuidedDataset(encodings, concept_tensor, labels_tensor)
...
def plot_metrics(config, epochs, val_losses, val_f1_scores, val_precisions, val_recalls,
    ...
    ax1.set_title('Concept Guided Attention Training Metrics')
...
def evaluate(config, timestamp):
    ...
    :param config: ConceptGuidedAttentionConfig 配置对象（用于获取base_path）
    :param timestamp: 实验时间戳
    """
    experiment_dir = config.base_path / "experiments_concept_guided_attention" / timestamp
    ...
    # 输出到控制台
    print("\n" + "=" * 30)
    print("      Concept Guided Attention 测试集评估结果")
    print("=" * 30)
    ...
    with open(test_results_dir / "classification_report.txt", "w", encoding="utf-8") as f:
        f.write("Concept Guided Attention 测试集评估结果\n")
        f.write("=" * 30 + "\n")
...
def main():
    """
    参数加载逻辑：训练模式基于concept_guided_attention_config.py，使用命令行参数更新配置，并保存到config.json中
    ...
    if args.mode in ['all', 'train']:
        # 获取完整参数配置
        config = update_ConceptGuidedAttentionConfig(args)
